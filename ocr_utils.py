...
def group_results_by_line(raw_results, y_tolerance=20):
    ...
    Sử dụng running mean của y_center trong dòng hiện tại để tránh drift.
    ...
    lines = []
    current_line = []
    current_line_y_sum = 0.0  # Dùng running mean thay vì so sánh với phần tử đầu

    for box in boxes:
        if not current_line:
            current_line.append(box)
            current_line_y_sum = box['y_center']
        else:
            current_line_y_mean = current_line_y_sum / len(current_line)
            if abs(box['y_center'] - current_line_y_mean) <= y_tolerance:
                current_line.append(box)
                current_line_y_sum += box['y_center']
            else:
